...
public class JdbcPostDao implements PostDao {
    ...
    public void createPost(Post post) throws IOException {
        ...
        try {
            PreparedStatement statement = connection.prepareStatement("INSERT INTO posts ( user_id, image, created_at, description) VALUES (?,?,?,?)");
            statement.setInt(1, (int) post.getAuthor().getId());
    ...
    public void updatePost(Post post, int postId) throws IOException {
        ...
        try {
            ...
            PreparedStatement statement = connection.prepareStatement("UPDATE post SET (post_id, user_id, image, created_at, description) VALUES (?,?,?,?,?) WHERE post_id =?  ");
    ...
    public List<Post> getPostsByUser(User user) throws IOException {
        ...
        try {
            ...
            while (resultSet.next()) {
                post = Post.builder().setId(resultSet.getInt("post_id")).setAuthor(user)
                        .setImage(resultSet.getString("image"))
                        .setCreatedAt(Timestamp.valueOf(resultSet.getString("created_at")).toLocalDateTime()).build();
        ...
        try {
            ...
            while (resultSet.next()) {
                ...
                Comment comment = Comment.builder()
                        ...
                        .setCreateAt(Timestamp.valueOf(resultSet.getString("created_at")).toLocalDateTime()).build();
